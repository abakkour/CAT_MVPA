...
function boost_mvpa_localizer_eye(subjid,test_comp,scan,order,runnum)

if sum(runnum==[1 2])==0
    error('Run nunmber must be 1 or 2');
elseif sum(scan==[0 1])==0
    error('Scan nunmber must be 0 or 1');
elseif sum(order==[1 2])==0
    error('Order nunmber must be 1 or 2');
end
%check if run1 already run...
if runnum==1 && ~isempty(dir(['Output/' subjid '_boost_loc_run1*.mat']))
    error(['ERROR: run1 on subject ' subjid ' already run']);
end
...
if runnum==1
    ...
    [names,shuff_ind]=Shuffle([vars{1}(loc)' vars{1}(loc)' vars{1}(loc)']');
    q=[ones(1,24) repmat(2,1,24) repmat(3,1,24)];
    qs=q(shuff_ind);
    
    shuff_names{1}=names(1:36);
    shuff_q{1}=qs(1:36);
    shuff_names{2}=names(37:72);
    shuff_q{2}=qs(37:72);
    
    bidIndex=[vars{3}(loc)' vars{3}(loc)' vars{3}(loc)']';
    sbidIndex=bidIndex(shuff_ind);
    shuff_bidIndex{1}=sbidIndex(1:36);
    shuff_bidIndex{2}=sbidIndex(37:72);
    
    itemnameIndex=[vars{4}(loc)' vars{4}(loc)' vars{4}(loc)']';
    sitemnameIndex=itemnameIndex(shuff_ind);
    shuff_itemnameIndex{1}=sitemnameIndex(1:36);
    shuff_itemnameIndex{2}=sitemnameIndex(37:72);
    
    bid=[vars{5}(loc)' vars{5}(loc)' vars{5}(loc)']';
    sbid=bid(shuff_ind);
    shuff_bid{1}=sbid(1:36);
    shuff_bid{2}=sbid(37:72);
    
else
    file=dir([outpath subjid '_boost_loc_run1*.mat']);
    load([outpath, sprintf(file(length(file)).name)]);
    runnum=2;
end

respTime{runnum}=ones(36,1)*999;
...
fid1=fopen(['Output/' subjid '_boost_loc_run' num2str(runnum) '_' timestamp '.txt'], 'a');
fprintf(fid1,'subjid test_comp runnum runtrial itemname question onsettime RT response fixationtime bidindex itemnameIndex bid \n'); %write the header line
...
% Eyelink msg
% - - - - - - - 
Eyelink('Message',strcat('Localizer run ',sprintf('%02d',runnum),' Start = ',num2str(runStartTime)));


for Pos=1:36   % To cover all the items in one run.
    ...
    % Eyelink msg
    % - - - - - - -
    trialmessage=strcat('trial ',num2str(Pos));
    ...
    if shuff_q{runnum}(Pos)==2
        while noresp
            ...
            if keyIsDown && noresp
                ...
                % Eyelink msg
                % - - - - - - -
                rtstr = strcat('responsetime=',num2str(respTime{runnum}(Pos,1)));
    ...
    fprintf(fid1,'%s %s %d %d %s %d %d %d %s %.2f %d %d %f\n', subjid, test_comp, runnum, Pos, shuff_names{runnum}{Pos}, shuff_q{runnum}(Pos), actual_onset_time{runnum}(Pos,1), respTime{runnum}(Pos,1), keyPressed,  fix_time-runStartTime, shuff_bidIndex{runnum}(Pos,1), shuff_itemnameIndex{runnum}(Pos,1),shuff_bid{runnum}(Pos,1));
...
if dummymode==0
	movefile('recdata.edf',strcat('Output/', subjid,'_localizer_run_', sprintf('%02d',runnum), '_', timestamp,'.edf'));
end;


% save data to a file, with date/time stamp
%time=clock;
outfile=strcat(outpath, sprintf('%s_boost_loc_run%d_%s.mat',subjid,runnum,timestamp));
...
if runnum==1
    fprintf(['\n \n \n You just ran localizer run 1. Next you want to run \n \n boost_mvpa_localizer_eye(''' char(subjid) ''',''' test_comp ''',' num2str(scan) ',' num2str(order) ',2) \n \n \n']);
elseif runnum==2
    fprintf(['\n \n \n You just ran localizer run 2. Next you want to run \n \n boost_mvpa_train_demo(''' char(subjid) ''',''' test_comp ''',' num2str(scan) ',950,950) \n \n \n']);
end
ListenChar(0);
clear all;
