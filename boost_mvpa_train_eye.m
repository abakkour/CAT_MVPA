...
%%% Edited by Akram Bakkour 04-17-2014


function boost_mvpa_train_eye(subjid,test_comp,order,scan,LADDER1IN, LADDER2IN,runnum)

Screen('Preference', 'VisualDebugLevel', 0);
% output version
script_name='Boost MVPA: with 2 optimized SSD trackers for: High and Low';
script_version='1';
revision_date='04-17-14';
...
% Eyelink msg
% - - - - - - - 
Eyelink('Message',strcat('Training run ',sprintf('%02d',runnum),' Start = ',num2str(runStartTime)));
...
for Pos=1:64,   % To cover all the items in one run.
    ...
    CenterText(w,'+', white,0,0);
    Screen('TextSize',w, 60);
    fix_time=Screen(w,'Flip', image_start_time+1);
    ...
    while (GetSecs-fix_time< 0.5) % these are additional 500msec to monitor responses
        [pressed, firstPress] = KbQueueCheck(-1);
        if pressed && noresp
            findfirstPress=find(firstPress);
            respTime{runnum}(Pos,1) = firstPress(findfirstPress(1))-image_start_time;
...
% - - - - - - -
runEnd=GetSecs;
Eyelink('Message',strcat('run', sprintf('%02d',runnum),'End=',num2str(runEnd)));
...
if dummymode==0
    movefile('recdata.edf',strcat('Output/', subjid,'_training_run', sprintf('%02d',runnum), '_eyetracking_', timestamp,'.edf'));
end;
...
if runnum<6
    fprintf(['\n \n \n You just ran training run ' num2str(runnum) '. Next you want to run \n \n boost_mvpa_train_eye(''' subjid ''',''' test_comp ''',' num2str(order) ',' num2str(scan) ',950,950,' num2str(runnum+1) ') \n \n \n']);
else
